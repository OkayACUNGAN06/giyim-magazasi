...
const Admin = () => {
  ...
  useEffect(() => { if(activeTab==='products') fetchP(); if(activeTab==='users') fetchU(); }, [activeTab]);
  const fetchP = async () => { const res = await axios.get('/api/products'); setProducts(res.data); };
  const fetchU = async () => { const res = await axios.get('/api/users'); setUsers(res.data); };
  const toggleRole = async (u) => { const nr = u.role==='admin'?'user':'admin'; if(window.confirm(`Yetki değişsin mi?`)) { await axios.put(`/api/users/${u._id}/role`,{role:nr}); fetchU(); }};
  const handleFileChange = (e) => { const f = e.target.files[0]; if(f){ const r=new FileReader(); r.onloadend=()=>setForm({...form, img:r.result}); r.readAsDataURL(f); }};
  const handleSubmit = async (e) => { e.preventDefault(); const p={...form, img:form.img||"https://via.placeholder.com/150"}; if(editMode){await axios.put(`/api/products/${editId}`,p);}else{await axios.post('/api/products',p);} setForm({name:'',price:'',size:'',img:''}); document.getElementById('fileInput').value=""; fetchP(); };
  const handleDelete = async (id) => { if(window.confirm("Silinsin mi?")) { await axios.delete(`/api/products/${id}`); fetchP(); } };
  const handleEdit = (p) => { setEditMode(true); setEditId(p._id); setForm({name:p.name,price:p.price,size:p.size,img:p.img}); window.scrollTo(0,0); };

  return (
    <div className="container">
       <div className="admin-header"><h2>🛠️ Yönetim Paneli</h2><div><button className={activeTab==='products'?'tab-btn active':'tab-btn'} onClick={()=>setActiveTab('products')}>Ürünler</button><button className={activeTab==='users'?'tab-btn active':'tab-btn'} onClick={()=>setActiveTab('users')}>Kullanıcılar</button></div></div>
       {activeTab==='products' && (<><div className="admin-form-card"><form onSubmit={handleSubmit} className="admin-form"><input placeholder="Ürün Adı" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/><input placeholder="Fiyat" type="number" value={form.price} onChange={e=>setForm({...form,price:e.target.value})}/><input placeholder="Beden" value={form.size} onChange={e=>setForm({...form,size:e.target.value})}/><div style={{border:'1px dashed #ccc',padding:10}}><label>Foto:</label><input id="fileInput" type="file" onChange={handleFileChange}/><input placeholder="Veya URL" value={form.img} onChange={e=>setForm({...form,img:e.target.value})}/></div><button className="save-btn">{editMode?'Güncelle':'Ekle'}</button></form></div><table className="admin-table"><thead><tr><th>Resim</th><th>İsim</th><th>Fiyat</th><th>İşlem</th></tr></thead><tbody>{products.map(p=>(<tr key={p._id}><td><img src={p.img} width="50" style={{borderRadius:5}}/></td><td>{p.name}</td><td>{p.price} TL</td><td><button onClick={()=>handleEdit(p)} className="edit-action">Düzenle</button><button onClick={()=>handleDelete(p._id)} className="delete-action">Sil</button></td></tr>))}</tbody></table></>)}
       {activeTab==='users' && (<table className="admin-table"><thead><tr><th>Kullanıcı</th><th>Rol</th><th>İşlem</th></tr></thead><tbody>{users.map(u=>(<tr key={u._id}><td>{u.username}</td><td><span className={u.role==='admin'?'badge-admin':'badge-user'}>{u.role.toUpperCase()}</span></td><td>{u.username!=='admin' && (<button onClick={()=>toggleRole(u)} className={u.role==='admin'?'delete-action':'save-btn'} style={{padding:'5px 10px', fontSize:'0.8rem'}}>{u.role==='admin'?'Yetkisini Al':'Admin Yap'}</button>)}</td></tr>))}</tbody></table>)}
    </div>
  );
};
